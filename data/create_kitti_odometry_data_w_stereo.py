...
resolutions = {'low_res': {'height': 128, 'width': 448}, 'med_res': {'height': 192, 'width': 640},
               'high_res': {'height': 256, 'width': 832}}
...
for resolution in ['med_res']:
    ...
    for seq in sequences:
        ...
        for cam_num in ['02', '03']:
            ...
            i = 0
            if cam_num == '02':
                seq_info['intrinsics_left'] = np.array(data.calib.K_cam2).reshape((-1, 3, 3)).repeat(
                    len(data.cam2_files), 0)
                with concurrent.futures.ProcessPoolExecutor() as executor:
                    for filename, output in zip(data.cam2_files, executor.map(load_image, data.cam2_files)):
                        ...
                        new_filename = os.path.join(target_dir, filename.split(args.source_dir)[1].replace(
                            'sequences/', '').replace('/' + seq + '/',
                                                      seq_names[seq] + '_' + cam_num + '/').replace('.png',
                                                                                                          '.jpg'))
            ...
            if cam_num == '03':
                seq_info['intrinsics_left'] = np.array(data.calib.K_cam3).reshape((-1, 3, 3)).repeat(
                    len(data.cam3_files), 0)
                with concurrent.futures.ProcessPoolExecutor() as executor:
                    for filename, output in zip(data.cam3_files, executor.map(load_image, data.cam3_files)):
                        img, zoomx, zoomy, orig_img_width, orig_img_height = output
                        new_filename = os.path.join(target_dir, filename.split(args.source_dir)[1].replace(
                            'sequences/', '').replace('image_3', 'image_2').replace('/' + seq + '/', seq_names[
                            seq] + '_' + cam_num + '/').replace('.png', '.jpg'))
                        imageio.imwrite(new_filename, img)
                        seq_info['intrinsics_left'][i, 0] *= zoomx
                        seq_info['intrinsics_left'][i, 1] *= zoomy
