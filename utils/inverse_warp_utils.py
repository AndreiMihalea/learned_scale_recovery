...
def get_factor_from_coords(cam_coords, gt_cam_height, height, width, height_offset, width_offset):
    samples = cam_coords[:, 1, height - height_offset:height, width // 2 - width_offset:width // 2 + width_offset]
    samples = samples.reshape(samples.shape[0], -1)

    median = samples.mean(1)
    factor = gt_cam_height / median
